...
-- ANY, SOME -- 메인 쿼리의 조건식을 만족하는 서브쿼리의 결과가 하나 이상이라면 TRUE
-- ALL -- 메인쿼리의 조건식을 서브커리의 결과 모두가 만족하면 TRUE
...
--단일형 서브쿼리와 함수--
--20번 부서에 속한 사원 중 전체 사원의 평균 급여보다 높은 급여를 받는 사람의 소속 부서 정보를 함께 출력
SELECT E.EMPNO, E.ENAME, E.JOB, ESAL, DDEPTNO, D.DNAME, D.LOC
    FROM EMP E, DEPT D
...
--응용-- 각 부서 별 최고 그여와 동일한 급여를 받는 사원 정보 출력
